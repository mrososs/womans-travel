...
const { saveTrip, deleteTrip } = useAdminContent();
const notify = useNotify();

useHead(() => ({ title: `${t('admin.trips.title')} · ${t('brand')}` }));

const { data, pending, refresh } = useAsyncData('admin-trips', async () => {
  const { data, error } = await client.from('trips').select('*').order('created_at');
  if (error) throw error;
  return data ?? [];
});
const rows = computed(() => data.value ?? []);

/** Blank row with sensible defaults for the "add" flow. */
function blankTrip(): TripInsert {
  return {
    id: '', title_ar: '', title_en: '', region_ar: '', region_en: '',
    category_id: 'city', kind: 'intl', tier_key: 'popular', tier_variant: 'gold',
    duration_ar: '', duration_en: '', dates_ar: '', dates_en: '',
...
const form = reactive<TripInsert>(blankTrip());
const isNew = ref(true);
const open = ref(false);
const saving = ref(false);

function openAdd() {
  Object.assign(form, blankTrip());
  isNew.value = true;
  open.value = true;
}
function openEdit(row: Database['public']['Tables']['trips']['Row']) {
  Object.assign(form, blankTrip(), row);
  isNew.value = false;
  open.value = true;
}

/** Numeric fields arrive as '' when cleared — normalise before upsert. */
...
async function confirmDelete() {
  ...
  try {
    await deleteTrip(deleteTarget.value);
  ...
  <div class="adminlist">
    <Card variant="elevated" padding="lg">
      <div class="adminlist__head">
        <h2 class="adminlist__title">{{ t('admin.trips.title') }}</h2>
          ...
          <tbody>
            <tr v-for="row in rows" :key="row.id">
              ...
              <td>
                ...
                <div class="adminlist__sub">{{ pick(row, 'region') }} · {{ row.id }}</div>
              </td>
            ...
            <tr v-if="!rows.length">
              <td colspan="5" class="adminlist__empty">{{ t('admin.emptyTrips') }}</td>
            </tr>
          </tbody>
        </table>
      </div>
